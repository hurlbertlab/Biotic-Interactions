...
comp_abun_data = left_join(focal_AOU, bbs_ep, by = c('focalAOU'= 'spAOU')) %>% 
  left_join(temp_occ[temp_occ$Aou %in% focal_and_comp_species, ], 
            by = c('focalAOU' = 'Aou', 'stateroute' = 'stateroute')) %>%
  left_join(bbs_ep, by = c('CompAOU' = 'spAOU', 'stateroute' = 'stateroute'))
comp_abun_data$focal_abun <- comp_abun_data$abundance.x
comp_abun_data$comp_abun <- comp_abun_data$abundance.y
# write.csv(comp_abun_data, "Data/comp_abun_data.csv", row.names = FALSE)
